...
const Agents = () => {
  ...
  return (
    <>
          ...
          <div className="grid lg:grid-cols-7 ">
            ...
            >
              {agents.map((agent, index) => (
                <motion.div
                  key={agent.id}
                  className={twMerge(
                    `flex cursor-pointer lg:flex-col md:flex-col-reverse flex-col-reverse `,
                ...
                >
                  ...
                  <motion.p
                    ...
                    onClick={() => {
                      fetchSelectedAgent(agent.uuid);
                      setIsActive(agent.uuid);
                    }}
                    animate={{ x: 0 }}
                    whileHover={{ x: "40px" }}
                  >
            ...
            <div className="lg:col-span-4 md:col-span-7 col-span-7 lg:my-0 md:my-7 my-7">
              <div className="w-full h-full mt-10">
                ...
                {selectAgent ? (
                  <motion.div
                    className="shadow-lg shadow-slate-800 rounded-2xl md:h-[90vh] h-[100%] w-full p-6 bg-white"
                    initial={{ opacity: 0, y: 6 }}
                    animate={{ opacity: 1, y: 1 }}
                    transition={{ duration: 1 }}
                  >
                    <motion.div
                      className="grid grid-cols-7 gap-4 md:text-base text-[5px]"
                      key={selectAgent.uuid}
                    >
                      <motion.div
                        className="col-span-4 rounded-xl  bg-red flex items-center justify-center"
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                      ...
                      <div className="col-span-3 ">
                        <motion.div
                          className="rounded-xl  bg-grey p-4 mb-4"
                        ...
                        >
                          <div className="bg-black flex flex-col items-center justify-center rounded-lg w-full h-[40%]">
                            ...
                            <p className="text-white mx-auto ">
                              {selectAgent.role.displayName}
                            </p>
                          </div>
                          <p className="text-black mt-2">
                            {selectAgent.role.description}
                          </p>
                        </motion.div>
                        <motion.div
                          className="col-span-7 rounded-xl  bg-black p-4"
                          initial={{ x: 10, opacity: 0 }}
                          animate={{ x: 0, opacity: 1 }}
                          transition={{ duration: 0.5, delay: 0.8 }}
                        >
      ...
      {selectAgent && <SkillSection abilities={selectAgent.abilities} />}
    </>
  );
};
